    ...
    reader.onload = (e) => {
      ...
      if (fileType === '5') {
        ...
        jsonData.forEach((row, rowIndex) => {
          ...
          if (currentIndividual) {
            if (row[0].includes('Goal Title:'))
              currentIndividual['Goal Title'] =
                jsonData[rowIndex + 1][0]?.trim() || ''
            if (row[2]?.includes('Goal Type:'))
              currentIndividual['Goal Type'] =
                jsonData[rowIndex + 1][2]?.trim() || ''
            if (row[3]?.includes('Personal Outcome:'))
              currentIndividual['Personal Outcome'] =
                jsonData[rowIndex + 1][3]?.trim() || ''
            if (row[6]?.includes('Start Date:'))
              currentIndividual['Start Date'] =
                jsonData[rowIndex + 1][6]?.trim() || ''
            if (row[8]?.includes('Completion Date:'))
              currentIndividual['Completion Date'] =
                jsonData[rowIndex + 1][8]?.trim() || ''
            if (row[9]?.includes('Discontinued Date:'))
              currentIndividual['Discontinued Date'] =
                jsonData[rowIndex + 1][9]?.trim() || ''
            if (row[0].includes('Goal Description:'))
              currentIndividual['Goal Description'] =
                jsonData[rowIndex][3]?.trim() || ''
            if (row[3]?.includes('Goal Progress:'))
              currentIndividual['Goal Progress'] =
                jsonData[rowIndex][3]?.trim() || ''
          }
        })
        ...
        jsonData.forEach((row, rowIndex) => {
          ...
          const individual: Type2Individual = {
            Individual:
              (typeof row[0] === 'string' ? row[0]?.trim() : '') || '', // Column A
            'Program or Site':
              (typeof row[1] === 'string' ? row[1]?.trim() : '') || '', // Column B
            'Start Date': formatDate(row[2]), // Convert date from Column C
            'Exit Date': formatDate(row[3]), // Convert date from Column D
            'Exit Reason':
              (typeof row[4] === 'string' ? row[4]?.trim() : '') || '' // Column E
          }
              ...
              (typeof row[4] === 'number'
                ? row[4] === 1
                  ...
                  : row[4] === 0
                    ? 'No'
                    : ''
                : row[4]?.trim()) || '', // Column E (0 or 1 -> Yes or No)
            'Reason and Rationale for Restriction':
              (typeof row[5] === 'string' ? row[5]?.trim() : '') || '', // Column F
            'Was this related to a critical incident':
              (typeof row[6] === 'string' ? row[6]?.trim() : '') || '', // Column G (Yes or No)
            'Staff Reporting':
              (typeof row[7] === 'string' ? row[7]?.trim() : '') || '', // Column H
            'Rationale for LOS > 48 hours':
              (typeof row[8] === 'string' ? row[8]?.trim() : '') || '', // Column I
            'Manager Approved':
              (typeof row[9] === 'number'
                ? row[9] === 1
                  ? 'Yes'
                  : row[9] === 0
                    ? 'No'
                    : ''
                : row[9]?.trim()) || '' // Column J (0 or 1 -> Yes or No)
          }
        ...
        jsonData.forEach((row, rowIndex) => {
          ...
          const individual: Type4Individual = {
            Individual:
              (typeof row[0] === 'string' ? row[0]?.trim() : '') || '', // Column A: Individual
            'Program or Site':
              (typeof row[1] === 'string' ? row[1]?.trim() : '') || '', // Column B: Program or Site
        ...
        jsonData.forEach((row, rowIndex) => {
          ...
          const individual: Type6Individual = {
            ...
            'Self-soothing strategies':
              (typeof row[2] === 'string' ? row[2]?.trim() : '') || '', // Column C: Self-soothing strategies
            'Reasons for living':
              (typeof row[3] === 'string' ? row[3]?.trim() : '') || '', // Column D: Reasons for living
            'Support connections':
              (typeof row[4] === 'string' ? row[4]?.trim() : '') || '', // Column E: Support connections
            'Safe spaces':
              (typeof row[5] === 'string' ? row[5]?.trim() : '') || '' // Column F: Safe spaces
          }
        ...
        jsonData.forEach((row, rowIndex) => {
          ...
          const individual: Type7Individual = {
            ...
            'Program or Site':
              (typeof row[1] === 'string' ? row[1]?.trim() : '') || '', // Column B: Program or Site
            'Staff Member':
              (typeof row[2] === 'string' ? row[2]?.trim() : '') || '', // Column C: Staff Member(s) completing form
            "Today's Date": formatDate(row[3]), // Column D: Today's Date
            'Risk Factors':
              (typeof row[4] === 'string' ? row[4]?.trim() : '') || '', // Column E: Risk Factors
            'Risk Reduction Actions':
              (typeof row[5] === 'string' ? row[5]?.trim() : '') || '', // Column F: Risk Reduction Actions
            'Wellness Habits':
              (typeof row[6] === 'string' ? row[6]?.trim() : '') || '', // Column G: Wellness Habits
            'Support People':
              (typeof row[7] === 'string' ? row[7]?.trim() : '') || '', // Column H: Support People
            'Crisis Contacts':
              (typeof row[8] === 'string' ? row[8]?.trim() : '') || '' // Column I: Crisis Contacts
          }
        ...
        jsonData.forEach((row, rowIndex) => {
          ...
          const incident: Type8Incident = {
            'Client(s) involved':
              (typeof row[0] === 'string' ? row[0]?.trim() : '') || '', // Column A: Client(s) involved
            'Program or Site':
              (typeof row[1] === 'string' ? row[1]?.trim() : '') || '', // Column B: Program or Site
            'Date and Time of Incident': formatDate(row[2]), // Convert date from Column C: Date and Time of Incident
            'Degree of injury':
              (typeof row[3] === 'string' ? row[3]?.trim() : '') || '', // Column D: Degree of injury
            'Type of injury':
              (typeof row[4] === 'string' ? row[4]?.trim() : '') || '', // Column E: Type of injury
            'Type of serious incident':
              (typeof row[5] === 'string' ? row[5]?.trim() : '') || '' // Column F: Type of serious incident
          }
        ...
        jsonData.forEach((row, rowIndex) => {
          ...
          const individual: Type10Individual = {
            ...
            'Current Goals':
              (typeof row[2] === 'string' ? row[2]?.trim() : '') || '', // Column C: Current Goals
            'Current Goals Description':
              (typeof row[3] === 'string' ? row[3]?.trim() : '') || '', // Column D: Current Goals Description
            'Follow-up log':
              (typeof row[4] === 'string' ? row[4]?.trim() : '') || '', // Column E: Follow-up log
            'Referral log':
              (typeof row[5] === 'string' ? row[5]?.trim() : '') || '', // Column F: Referral log
            'Successful Diversion':
              (typeof row[6] === 'string' ? row[6]?.trim() : '') || '', // Column G: Successful Diversion
            'Diverted To':
              (typeof row[7] === 'string' ? row[7]?.trim() : '') || '', // Column H: Diverted To
            'Diversion Method':
              (typeof row[8] === 'string' ? row[8]?.trim() : '') || '', // Column I: Diversion Method
            'Diversion Cost':
              (typeof row[9] === 'string' ? row[9]?.trim() : '') || '', // Column J: Diversion Cost
            'Eviction Prevention':
              (typeof row[10] === 'string' ? row[10]?.trim() : '') || '' // Column K: Eviction Prevention
          }
    ...
    <GradientBackgroundContainer className="min-h-screen min-w-full flex items-center justify-center">
      <div className="flex flex-col items-center justify-center w-full max-w-2xl px-4 gap-8">
        ...
        <div className="w-full space-y-4">
          <dl className="text-md/6 font-medium text-gray-900">
            5 recently upload
          </dl>
            ...
            >
                ...
                <div className="flex w-0 flex-1 items-center">
                  ...
                  <div className="ml-4 flex min-w-0 flex-1 gap-2">
                    <span className="truncate font-medium">
                      resume_back_end_developer.excel
                    </span>
                    <span className="shrink-0 text-gray-400">Jun-01-2025</span>
                  </div>
                </div>
              ...
              <li className="flex items-center justify-between py-4 pr-5 pl-4 text-sm/6">
                <div className="flex w-0 flex-1 items-center">
                  ...
                  <div className="ml-4 flex min-w-0 flex-1 gap-2">
                    <span className="truncate font-medium">
                      coverletter_back_end_developer.excel
                    </span>
                    <span className="shrink-0 text-gray-400">Jun-01-2025</span>
                  </div>
                </div>
